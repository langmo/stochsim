...
namespace expression
{
	...
	/// </summary>
	class ConjunctionExpression : public IVariadicExpression
	{
	public:
		static constexpr number nullElement = number_true;
	public:
		ConjunctionExpression(number baseValue = nullElement) : IVariadicExpression(baseValue)
		{
		}
		...
		virtual std::unique_ptr<IExpression> Simplify(const VariableLookup& variableLookup) const override
		{
			...
			for (auto& elem : elems_)
			{
				...
				else if (elem.IsNotInverse() && dynamic_cast<ConjunctionExpression*>(simElem.get()))
				{
					auto& subExp = *static_cast<ConjunctionExpression*>(simElem.get());
			...
			{
				auto returnVal = std::make_unique<ConjunctionExpression>(number_true);
		...
		{
			auto val = std::make_unique<ConjunctionExpression>(baseValue_);
			for (auto& elem : elems_)
